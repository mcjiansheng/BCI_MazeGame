...
def main() -> int:
    ...
    capacity = int(args.display_seconds * args.sample_rate)
    display = np.empty((len(args.channel_names), 0), dtype=np.float64)
    ...
    if not args.no_plot:
        ...
        lines = [axes.plot([], [], linewidth=0.8, label=name)[0] for name in args.channel_names]
    ...
    try:
        with source:
            ...
            print(f"已连接 backend={args.backend}, {source.sample_rate} Hz, 通道={source.channel_names}")
            while time.monotonic() - started < args.duration:
                ...
                now = time.monotonic()
                if now >= next_quality and display.shape[1] >= args.sample_rate * 2:
                    quality = assess_signal_quality(display, args.sample_rate)
                    print("\n" + format_quality_table(tuple(args.channel_names), quality))
                    next_quality = now + 2.0
                if figure is not None and lines is not None and display.shape[1]:
                    x = np.arange(display.shape[1]) / args.sample_rate - display.shape[1] / args.sample_rate
    ...
    rate_error = abs(effective_rate - args.sample_rate) / args.sample_rate
    print(
        f"采样核对：收到 {continuous.shape[1]} 点 / {elapsed:.2f}s = "
        f"{effective_rate:.1f} Hz（目标 {args.sample_rate} Hz）"
    )
    if rate_error > 0.15:
        print("警告：实际接收速率与目标相差超过 15%，请检查设备采样率命令、丢包或 LSL 配置。")
    final_quality = assess_signal_quality(continuous[:, -min(continuous.shape[1], capacity) :], args.sample_rate)
    print("\n最终质量摘要\n" + format_quality_table(tuple(args.channel_names), final_quality))
    if args.output:
